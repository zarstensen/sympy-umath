...
Adds `umath` as a possible value to sympy's `lambdify` function, which uses the uncertainties package as the backend.
Also adds a `lambdify_umath` function which passes this automatically.
